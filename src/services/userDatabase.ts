// src/services/userDatabase.ts - Node.js compatible version (no bun:sqlite)
...
class UserDatabase {
  ...
  private generateFallbackSecret(): string {
    ...
    console.warn(
      "⚠️  Generated temporary session secret - sessions will be invalidated on restart"
    );
    ...
    try {
      ...
      if (parts.length === 4) {
        ...
        if (oldParts.length >= 3) {
          return { userId: oldParts[0], timestamp: parseInt(oldParts[1]) };
        }
      }
      return null;
    } catch {
  ...
  getUserDefaultSMTPConfig(userId: string): UserSMTPConfig | null {
    return this.db
      .prepare(
        `SELECT * FROM user_smtp_configs WHERE user_id = ? AND is_default = 1`
      )
  ...
  cleanExpiredSessions(): void {
    ...
    if ((result as any).changes > 0) {
      console.log(`🧹 Cleaned ${(result as any).changes} expired sessions`);
    }
  }
}
